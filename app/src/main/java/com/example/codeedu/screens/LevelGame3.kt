...
/**Functionality for level 1 game 3- includes the game content ui*/
@Composable
fun LevelGame3(
    level: Int,
    ...
    val commandSequence = remember { mutableStateListOf<Command?>(null, null, null, null, null, null) }
    ...
    val targetPosition = Offset(275f, 495f)

    val successConditionMet = (animatedPosition.x in targetPosition.x - 50f..targetPosition.x + 50f) &&
            (animatedPosition.y in targetPosition.y - 50f..targetPosition.y + 50f)

    Column(modifier = Modifier.fillMaxSize().background(Color(0xFFE0F7FA)).padding(5.dp)) {
        ...
        Text("YOUR SEQUENCE-LEVEL 1 GAME 3", style = MaterialTheme.typography.titleMedium)
        FlowRow(
            modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp),
            horizontalArrangement = Arrangement.SpaceEvenly,
            maxItemsInEachRow = 6
